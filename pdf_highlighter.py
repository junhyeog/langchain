...
def extract_info(input_file: str):
    ...
    output = {
        "File": input_file,
        "Encrypted": ("True" if pdfDoc.is_encrypted else "False"),
    }
...
def highlight_matching_data(page, matched_values, type):
    ...
    for val in matched_values:
        ...
        # highlight.set_colors({"stroke": (1, 1, np.random.rand()), "fill": (1, 1, 0)})
        colors = [
            # "red",
            "green",
            # "blue",
        ...
        ops = [0.1, 0.4, 0.7, 1.0]
        highlight.set_opacity(np.random.choice(ops))
        # highlight.set_opacity(0.1)
        print(highlight.colors, highlight.opacity)
        # highlight.set_colors({"stroke": (0, 1, 1, 0)})
        # highlight.set_colors(
        #     stroke=fitz.utils.getColor("blue"), fill=fitz.utils.getColor("red")
        # )
